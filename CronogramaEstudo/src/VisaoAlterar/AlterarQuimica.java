...
public class AlterarQuimica extends javax.swing.JFrame {

    public AlterarQuimica() {
        initComponents();
        setSize(781,410);
    }
     private void InserirDados(int cod) {
        Connection con = Conexao.AbrirConexao();
        QuimicaDAO sql = new QuimicaDAO(con);
        List<Quimica> lista = new ArrayList<>();
        lista = sql.CapturarQuimica(cod);
        for (Quimica q : lista) {
            conteudo.setText("" + q.getConteudo());
            data.setText("" + q.getData());
            hora.setText("" + q.getHora());
            Fcod.setText(""+ q.getId());
        }
    ...
    private void okActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_okActionPerformed
        ...
        if (sql.TestarQuimica(cod) == false) {
