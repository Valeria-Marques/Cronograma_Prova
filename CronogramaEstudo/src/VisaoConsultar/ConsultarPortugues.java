...
public class ConsultarPortugues extends javax.swing.JFrame {

    public ConsultarPortugues() {
        initComponents();
        setSize(781,410);
          AtualizaTable();
    }
     ...
     public void BuscarNome(){
        ...
        PortuguesDAO bd =  new PortuguesDAO(con);
        List<Portugues> lista = new ArrayList();
        lista = bd.PesquisarNomePortugues(nome.getText());
        DefaultTableModel tbm = (DefaultTableModel) Tabela.getModel();
        while(tbm.getRowCount() > 0){
            tbm.removeRow(0);
        }
   ...
   dispose();
    ...
    private void BCod1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BCod1ActionPerformed
      String codigo = cod.getText();
        Connection con = Conexao.AbrirConexao();
        PortuguesDAO sql = new PortuguesDAO(con);
        List<Portugues> lista = new ArrayList<>();
        lista = sql.PesquisarCodigoPortugues(codigo);
        DefaultTableModel tbm = (DefaultTableModel) Tabela.getModel();
        while (tbm.getRowCount() > 0) {
            tbm.removeRow(0);
        }
