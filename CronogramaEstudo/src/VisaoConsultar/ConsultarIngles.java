...
public class ConsultarIngles extends javax.swing.JFrame {

    public ConsultarIngles() {
        initComponents();
        setSize(781,410);
          AtualizaTable();
    }
     ...
     public void BuscarNome(){
        ...
        InglesDAO bd =  new InglesDAO(con);
        List<Ingles> lista = new ArrayList();
        lista = bd.PesquisarNomeIngles(nome.getText());
        DefaultTableModel tbm = (DefaultTableModel) Tabela.getModel();
        while(tbm.getRowCount() > 0){
            tbm.removeRow(0);
        }
   ...
   dispose();
    ...
    private void BCod1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BCod1ActionPerformed
      String codigo = cod.getText();
        Connection con = Conexao.AbrirConexao();
        InglesDAO sql = new InglesDAO(con);
        List<Ingles> lista = new ArrayList<>();
        lista = sql.PesquisarCodigoIngles(codigo);
        DefaultTableModel tbm = (DefaultTableModel) Tabela.getModel();
        while (tbm.getRowCount() > 0) {
            tbm.removeRow(0);
        }
