...
public class ConsultarRedes extends javax.swing.JFrame {

    public ConsultarRedes() {
        initComponents();
        setSize(781,410);
          AtualizaTable();
    }
     private void AtualizaTable() {
        ...
        RedesDAO bd = new RedesDAO(con);
        List<Redes>lista = new ArrayList<>();
        lista = bd.ListarRedes();
     ...
     public void BuscarNome(){
        ...
        RedesDAO bd =  new RedesDAO(con);
        List<Redes> lista = new ArrayList();
        lista = bd.PesquisarNomeRedes(nome.getText());
        DefaultTableModel tbm = (DefaultTableModel) Tabela.getModel();
        while(tbm.getRowCount() > 0){
            tbm.removeRow(0);
        }
   ...
   dispose();
    ...
    private void BCod1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BCod1ActionPerformed
      String codigo = cod.getText();
        Connection con = Conexao.AbrirConexao();
        RedesDAO sql = new RedesDAO(con);
        List<Redes> lista = new ArrayList<>();
        lista = sql.PesquisarCodigoRedes(codigo);
        DefaultTableModel tbm = (DefaultTableModel) Tabela.getModel();
        while (tbm.getRowCount() > 0) {
            tbm.removeRow(0);
        }
